...
/**
 * First true stream-fed MySQL runtime path.
 *
 * <p>Unlike the buffered inspection shell, this runtime reads committed transactions from a live
 * session collaborator instead of manual enqueue calls.
 */
public final class MySqlLiveStreamingRuntime
    implements WatermarkWindowRuntime<MySqlBinlogTransaction>, RuntimeStatusInspectable {
  ...
  private final MySqlTransactionStreamingSession session;
  ...
  public MySqlLiveStreamingRuntime(
  ...
  public String lastAcknowledgedCheckpointDisplayValue() {
    return checkpointStore.load(sourceId).map(MySqlSourcePosition::displayValue).orElse(null);
  }

  @Override
  public int pendingTransactionCount() {
    // The MySQL adapter reads binlog events on a background thread and hands them to the
    // orchestrator via a BoundedSourceEventQueue. Pending count is the queue depth — the number
    // of committed binlog messages already staged for the consumer but not yet drained.
    return session.sourceFlowControlSnapshot().queueDepth();
  }
