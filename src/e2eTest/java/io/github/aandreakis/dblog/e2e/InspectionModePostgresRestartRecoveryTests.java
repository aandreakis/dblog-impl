...
class InspectionModePostgresRestartRecoveryTests {
  @TempDir Path tempDir;

  @Test
  void resumesAnActiveTableDumpAfterRestartFromTheLastCompletedBoundary() throws Exception {
    String h2Jdbc = "jdbc:h2:mem:phase30_postgres_restart;MODE=PostgreSQL;DB_CLOSE_DELAY=-1";
    TableSchema schema =
        TableSchema.create(
    ...
    RelationalSourceConfig config =
        new RelationalSourceConfig(
            ...
            java.util.Map.of(),
            false);
    Path statePath = tempDir.resolve("phase30-postgres-restart-state");
